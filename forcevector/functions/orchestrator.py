...
def execute_autonomous_flow(target: str, config_data: dict, db_conn, logs_path: Path, skip_recon: bool = False):
    ...
    print(f"\n{COLOR_INFO}=== INICIANDO FASE 3: MODELADO DE AMENAZAS (LLM Iterativo + RAG) ==={COLOR_RESET}")
    ...
    if db_conn:
        try:
            ...
            for idx, vec in enumerate(vectores_crudos, 1):
                ...
                # --- INICIO DE CONEXIÓN RAG (HACIA WINDOWS SERVER) ---
                print(f"{COLOR_YELLOW}[*] Consultando Memoria Vectorial (PostgreSQL) por CVEs y exploits de {srv}...{COLOR_RESET}")
                contexto_db = "No se encontraron CVEs relevantes en la base de datos."
                try:
                    with db_conn.cursor() as cur_rag:
                        # Consulta estructurada usando cve_descriptions para extraer la columna 'value'
                        query = """
                        SELECT d.cve_id, d.value 
                        FROM cve_descriptions d
                        WHERE d.value ILIKE %s OR d.value ILIKE %s
                        LIMIT 5;
                        """
                        # Limpiamos términos para una mejor coincidencia
                        termino_srv = f"%{srv.strip()}%"
                        # Si no hay versión, evitamos buscar "%Desconocido%" que daría falsos positivos
                        termino_ver = f"%{ver.strip()}%" if ver and ver != "No especificada" else termino_srv
                        
                        cur_rag.execute(query, (termino_srv, termino_ver))
                        resultados = cur_rag.fetchall()
                        
                        if resultados:
                            contexto_db = "DATOS EMPÍRICOS DE TU BASE DE DATOS LOCAL:\n"
                            for row in resultados:
                                # row[0] es el cve_id, row[1] es la descripción (value)
                                contexto_db += f"- CVE: {row[0]} | Info: {row[1][:150]}...\n"
                            print(f"{COLOR_SUCCESS}[+] RAG Hit: {len(resultados)} registros inyectados desde Windows Server.{COLOR_RESET}")
                        else:
                            print(f"{COLOR_YELLOW}[-] RAG: No hubo coincidencias SQL para {srv}.{COLOR_RESET}")
                except Exception as e:
                    print(f"{COLOR_ERROR}[!] Advertencia DB: {e}. Asegúrate de que las tablas cves y cve_descriptions existan.{COLOR_RESET}")
                # --- FIN DE CONEXIÓN RAG ---

                prompt_iterativo = (
                    "Analiza el siguiente vector de ataque.\n"
                    "REGLAS CRÍTICAS: Devuelve ÚNICA Y EXCLUSIVAMENTE formato JSON. "
                    "Correlaciona la versión con identificadores CVE reales basándote ESTRICTAMENTE en el conocimiento RAG proporcionado a continuación.\n"
                    f"CONOCIMIENTO RAG EXTRAÍDO: {contexto_db}\n\n"
...
def execute_exploitation_phase(target_ip: str, config_data: dict, logs_path: Path):
    ...
    try:
        ...
        # FIX: Eliminamos el gatillo automático de la Fase 6 para evitar doble ejecución.
        # El operador decidirá si ejecutar 'post <IP>' desde el Sandbox o desde el menú principal.
        print(f"\n{COLOR_SUCCESS}[+] Fases 4 y 5 completadas. Flujo de explotación finalizado.{COLOR_RESET}")
...
def execute_post_exploitation_phase(target_ip: str, config_data: dict):
    ...
    try:
        ...
        run_post_exploitation(target_ip, config_data)
    except Exception as e:
